...
func main() {
	...
	// test https call
	http.Handle("/https:/", makeInstrumentedHandler(httpsProxyHandler, "httpsProxy"))

	// test https call
	http.Handle("/http:/", makeInstrumentedHandler(httpProxyHandler, "httpProxy"))
...
func makeInstrumentedHandler(handler http.HandlerFunc, handlerLabel string) http.Handler {
	return promhttp.InstrumentHandlerDuration(durationHisVec.MustCurryWith(prometheus.Labels{"handler": handlerLabel}),
		handler,
	)
}
...
func rootHandler(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Fprintf(w, "%v %v %v\n\n", os.Getenv("POD_NAME"), os.Getenv("POD_NAMESPACE"), os.Getenv("POD_IP"))
...
func webCall(w http.ResponseWriter, r *http.Request, protocol string) {
	log.Printf("%s %s", r.Method, r.URL.Path)
	offset := len(protocol) + 3
	url := protocol + "://" + fmt.Sprint(r.URL)[offset:]
